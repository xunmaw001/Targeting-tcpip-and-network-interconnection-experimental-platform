	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="互评学号" prop="hupingxuehao">
					<el-input v-model="ruleForm.hupingxuehao" placeholder="互评学号" clearable  :readonly="ro.hupingxuehao"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="互评学号" prop="hupingxuehao">
					<el-input v-model="ruleForm.hupingxuehao" placeholder="互评学号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="互评姓名" prop="hupingxingming">
					<el-input v-model="ruleForm.hupingxingming" placeholder="互评姓名" clearable  :readonly="ro.hupingxingming"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="互评姓名" prop="hupingxingming">
					<el-input v-model="ruleForm.hupingxingming" placeholder="互评姓名" readonly></el-input>
				</el-form-item>
			</template>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='hupingxuehao'){
							this.ruleForm.hupingxuehao = obj[o];
							this.ro.hupingxuehao = true;
							continue;
						}
						if(o=='hupingxingming'){
							this.ruleForm.hupingxingming = obj[o];
							this.ro.hupingxingming = true;
							continue;
						}
						if(o=='crossuserid'){
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					if(((json.xuehao!=''&&json.xuehao) || json.xuehao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.hupingxuehao = json.xuehao
						this.ro.hupingxuehao = true;
					}
					if(((json.xingming!=''&&json.xingming) || json.xingming==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.hupingxingming = json.xingming
						this.ro.hupingxingming = true;
					}
				} else {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xueshenghuping/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xueshenghupingCrossAddOrUpdateFlag = false;
						 this.parent.search();
