	...
	<div class="addEdit-block" :style='{"padding":"10px 30px 30px","background":"url(http://codegen.caihongy.cn/20230228/47752d6be3a94e8892a83ec2a70c8443.png)"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.shiyanbaogao" label="实验报告" prop="shiyanbaogao">
					<file-upload
						tip="点击上传实验报告"
						action="file/upload"
						:limit="1"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="实验过程" prop="shiyanguocheng">
					<el-input
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shiyanguocheng" label="实验过程" prop="shiyanguocheng">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.shiyanguocheng}}</span>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="实验结果" prop="shiyanjieguo">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shiyanjieguo" label="实验结果" prop="shiyanjieguo">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.shiyanjieguo"></span>
                </el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="实验总结" prop="shiyanzongjie">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.shiyanzongjie" label="实验总结" prop="shiyanzongjie">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.shiyanzongjie"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shiyanjieguo = this.ruleForm.shiyanjieguo.replace(reg,'../../../ssmji1s8/upload');
        this.ruleForm.shiyanzongjie = this.ruleForm.shiyanzongjie.replace(reg,'../../../ssmji1s8/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shiyanbaogao!=null) {
		this.ruleForm.shiyanbaogao = this.ruleForm.shiyanbaogao.replace(new RegExp(this.$base.url,"g"),"");
	}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `baogaozhanshi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.baogaozhanshiCrossAddOrUpdateFlag = false;
						 this.parent.search();
